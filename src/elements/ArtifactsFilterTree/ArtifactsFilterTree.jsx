...
const ArtifactFilterTree = ({ filterTreeOptions, label, match, onChange, page, value }) => {
    ...
    event => {
      if (!event.path.includes(artifactFilterTreeRef.current)) {
        if (filterTree.length <= 0) {
          onChange('latest')
          setFilterTree(filterTreeOptions.find(tree => tree.id === 'latest').label)
        } else {
  ...
  const handleSelectFilter = tree => {
    if (tree.id !== filterTree) {
      if (match.params.jobId || match.params.name) {
        history.push(`/projects/${match.params.projectName}/${page.toLowerCase()}`)
      }
  ...
  return (
    ...
    >
      ...
      />
      <div className="drop_down" onClick={() => setIsDropDownMenu(!isDropDownMenuOpen)}>
        <Caret />
      </div>
      {isDropDownMenuOpen && (
