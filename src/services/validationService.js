...
export const validationService = {
  generateRule: {
    ...
    beginNotWith: chars => {
      return {
        name: 'beginNot',
        label:
          ValidationConstants.BEGIN_NOT_WITH + ': ' + convertToLabel(chars),
        pattern: new RegExp('^[^' + convertToPattern(chars) + ']')
      }
    },
    endWith: chars => {
    ...
    beginEndWith: chars => {
      ...
      return {
        name: 'beginEnd',
        label:
          ValidationConstants.BEGIN_END_WITH + ': ' + convertToLabel(chars),
        pattern: new RegExp(
   ...
   */
  getValidationRulesByType: (type, additionalRules) => {
    return lodash
      .chain(this.commonRules)
      .get(type)
      .defaultTo([])
      .cloneDeep()
