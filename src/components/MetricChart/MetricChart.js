...
const GenericMetricChart = ({ chartConfig, isInvocationCardExpanded }) => {
  ...
  const customPoints = useMemo(() => {
    ...
    const isOnlyOneValue = chartConfig?.data?.datasets[0]?.data?.length === 1
    ...
    return {
      radius: context => {
        ...
        return (hasDriftStatusList && isCurrentIndexTotalDriftIndex) || isOnlyOneValue ? 2 : 0
      },
      pointStyle: context => (hasDriftStatusList || isOnlyOneValue ? 'circle' : 'none'),
      backgroundColor: () => (hasDriftStatusList || isOnlyOneValue ? 'black' : undefined),
      borderColor: () => 'white'
    }
  }, [chartConfig])
